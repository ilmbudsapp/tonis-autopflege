 ...
 * converts to high-fidelity WebP in public/assets/images/brands/, and writes src/generated/brandLogos.ts.
 *
...
const generatedTs = path.join(root, "src", "generated", "brandLogos.ts");
...
async function main() {
  ...
  const entries = BRAND_RULES.filter((b) => chosen.has(b.slug)).map((b) => `  "${b.slug}": "${b.slug}.webp",`);

  const ts = `/** Auto-generated by scripts/build-brands-from-proizvodi.mjs — do not edit by hand. */
export type BrandLogoSlug = ${BRAND_RULES.map((b) => JSON.stringify(b.slug)).join(" | ")};
